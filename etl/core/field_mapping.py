...
FIELD_MAPPING = {
    ...
    "base": {
        # 基础信息字段
        "date": "日期",
        "brand_store_name": "品牌门店名称",     # 通过映射表获取
        "platform_store_name": "平台门店名称",  # 从源文件的"门店名称"列读取
        "store_id": "门店id",
        "city": "门店所在城市",
        ...
        "actual_income": "商家实收",
        "expense": "支出",
        "turnover": "营业额",           # 修正：使用优惠前总额作为营业额
        "net_margin_rate": "到手率",    # 到手率需要计算，原始数据无
        "original_price": "商品原价",
        "packaging_fee": "包装费",
        ...
        "real_net_margin_rate": "真实到手率", 

        # 订单数量字段
        "valid_orders": "有效订单",
        "invalid_orders": "无效订单",  # 使用取消订单作为无效订单
        ...
        # 回复率字段
        "five_min_reply_rate": "5分钟在线联系回复率",  # 5分钟回复率需要人工输入
        "one_min_reply_rate": "1分钟在线联系回复率",  # 1分钟回复率需要人工输入
    ...
    "meituan": {
        ...
        "service_negative_feedback_score": "服务负反馈率指标得分",
        "food_safety_negative_feedback_score": "食品安全负反馈率指标得分",
        "five_min_reply_rate": "近7日日均在线联系回复率",
        "one_min_reply_rate": None,
        "message_reply_rate": "消息回复率",
        "food_safety_negative_feedback_rate": "食品安全负反馈率",
        "basic_duration": "基础营业时长",
        "meal_completion_report_rate": "出餐完成上报率/配送准时率",
    },
    ...
    "eleme": {
        ...
        "valid_orders": "有效订单",
        "invalid_orders": "无效订单",
        "merchant_cancelled_orders": "商责取消数",
        "merchant_cancellation_rate": "商责取消率",
